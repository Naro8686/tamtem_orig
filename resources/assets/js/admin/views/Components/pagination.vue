  ...
  <div class="pagination-info">
    ...
    <template v-if="paginationsettings.viewpagination">
      <div class="pagination">
        <div class="pagination__controls">
          ...
          >
            <svg
              class="pagination__control-icon"
              viewBox="0 0 24 24"
              width="24"
              height="24"
            >
              <path
                d="M18.4 16.6L13.8 12l4.6-4.6L17 6l-6 6 6 6 1.4-1.4M6 6h2v12H6V6z"
              ></path>
            </svg>
          </button>
          <button
        ...
        <ul class="pagination__list">
          <template v-if="hasitems">
            <li
              ...
              :class="{
                'pagination__item--active': startPage + n - 1 == currentPage
              }"
            >
              <a
                class="pagination__link"
                v-on:click="updatepage(startPage + n - 1)"
                >{{ startPage + n - 1 }}</a
              >
            </li>
        ...
        <div class="pagination__controls">
          ...
          >
            ...
            >
              <path
                d="M5.6 7.4l4.6 4.6-4.6 4.6L7 18l6-6-6-6-1.4 1.4M16 6h2v12h-2V6z"
              ></path>
            </svg>
          </button>
        </div>
    ...
    <template v-if="paginationsettings.viewperpage">
      <div class="per_page--wrap">
        <p>
          <span class="elems-per-page">{{
            paginationsettings.elemsname | getname
          }}</span>
          на страницу:
        </p>
        <select
          v-on:change="changeperpage($event.target.value)"
          :value="perPage"
        >
          <option v-for="item in itemsArray" :key="item" :value="item">
            {{ item }}
          </option>
        </select>
      </div>
