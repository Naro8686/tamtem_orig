<template>
  <div class="box-body dataTables_wrapper">
      ...
      <thead>
        <tr>
          <th
            ...
            :class="[
              col.sort && sortKey == colSortKey(col)
                ? sortOrders[colSortKey(col)] > 0
                  ? 'sorting_asc'
                  : 'sorting_desc'
                : col.sort
                ? 'sorting'
            ...
            @click="col.sort ? sortBy(colSortKey(col)) : ''"
          >
            {{ col.name }}
          </th>
        </tr>
    ...
    <div class="box-footer clearfix">
      <router-link
        v-if="createRouteShow"
        :to="createRoute"
        class="btn btn-default"
        >{{ createText }}</router-link
      >
...
export default {
  ...
  computed: {
    createRouteShow() {
      if (this.createRoute == null) return false;
