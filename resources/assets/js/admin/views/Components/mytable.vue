  ...
  <div class="hello">
    <table class="my__table">
      <thead :class="[{ fixed: isFixedHead }, { noslot: !tableheadslot }]">
        <template v-for="col in fieldslist">
      ...
      <tbody>
        <template v-if="preloader">
          <tr class="empty-tr">
            <td :colspan="fieldslist.length" class="empty-td">
        ...
        <template v-else>
          ...
          <template v-else>
            <tr
              :class="itm.trClass"
              :key="itm[tablekey]"
              v-on:dblclick="trclick(itm)"
              v-for="itm in tabledata"
            >
              <template v-for="col in fieldslist">
...
//       required: true
//     },
//     fieldslist: {                    --  массив полей таблицы
//       type: Array
//     },
