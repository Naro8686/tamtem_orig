      ...
      <div class="box-body">
        <form @submit="saveForm">
          ...
          >
            <div class="col-md-4">
              <div
                :class="['form-group', errorsServer.geo_lat ? 'has-error' : '']"
              >
                ...
                <span
                  class="help-block"
                  v-if="errorsServer.geo_lat"
                  :errorsServer="errorsServer"
                >
                  {{ errorsServer.geo_lat[0] }}
                </span>
              </div>
            </div>
            <div class="col-md-4">
              <div
                :class="['form-group', errorsServer.geo_lon ? 'has-error' : '']"
              >
                <label for="geo_lon">Долгота</label>
                <input
                  v-model="item.geo_lon"
                  type="text"
                  class="form-control"
                  id="geo_lon"
                ...
                <span
                  class="help-block"
                  v-if="errorsServer.geo_lon"
                  :errorsServer="errorsServer"
                >
                  {{ errorsServer.geo_lon[0] }}
                </span>
              </div>
            </div>
            <div class="col-md-4">
              <div :class="['form-group', errorsServer.pos ? 'has-error' : '']">
          ...
          <div class="col-md-12 frm-buttons">
            <div class="btn-group">
              <input
                v-if="this.$root.profile.permissions.kladr.edit"
                class="btn btn-default"
                type="submit"
                value="Редактировать"
              />
              <input
                v-if="this.$root.profile.permissions.kladr.delete"
...
export default {
  ...
  methods: {
    ...
    loadItem() {
      axios
        .get(this.getItemApi(this.itemId))
        .then((resp) => {
          this.item = resp.data;
    ...
    deleteItem() {
      axios.delete(this.getItemApi(this.itemId)).then((resp) => {
    ...
    saveForm(event) {
      ...
      axios
        .patch(this.getItemApi(this.item.id), this.item)
        .then((resp) => {
          this.messageSaved();
        })
