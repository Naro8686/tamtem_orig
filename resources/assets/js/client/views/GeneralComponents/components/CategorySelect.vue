...
div.categories-select
	...
	v-select(
		...
		:error-messages="err.subcategories ? err.subcategories : errors.collect('subcategory')"
    ...
    data-vv-as='Подкатегория'
		data-vv-name='subcategory'
		return-object
		:light='true'
    :disabled="disabled"
      v-validate="'required|subcategorySelected'"
		solo
	)
	v-select(
...
export default {
  ...
  methods: {
    ...
    getCategories() {
      axios
        ...
        .then(resp => {
          ...
          Object.assign(
            this.categories,
            this.generateCatsArray(resp.data.data)
          );
    ...
    generateCatsArray(
      arr,
      label = "Категория" // Add object default category
    ) {
    ...
    getCategoryFormat(name = "Категории", id = null, items = null) {
      // For need format
      ...
      if (curCategory) {
        if (curCategory.parent_id == 0) {
          ...
          if (parentCategory.parent_id == 0) {
            ...
            // if not root parent category
            const parentSubCategory = this.getCatFromListById(
  ...
  created() {
    ...
    Validator.extend("subcategorySelected", {
      getMessage: field => "Необходимо выбрать подкатегорию.",
      validate: id => (this.subCategorySelected.id ? true : false)
    });
  },
  mounted() {
    this.getCategories();
...
<style lang="scss">
@import "~styleguide";
// специальные стили для выпадающего списка
.v-select-list {
  .v-list__tile__title {
