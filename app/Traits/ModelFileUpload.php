...
trait ModelFileUpload
{
    ...
    function uploadFileToStore($file, $path)
    {
        try{
  
            $arrToImportTable = [];
      
            $localStorePath = $file->store( $path );

            $arrToImportTable = [
                    'file_name' => $file->getClientOriginalName(),
                    'file_full_path' =>  config('filesystems.disks.local.prefix') . $localStorePath
            ];
    ...
    function deleteFileFromStoreOnPath($path)
    {
        try{
            return unlink(public_path($path));            
        }
